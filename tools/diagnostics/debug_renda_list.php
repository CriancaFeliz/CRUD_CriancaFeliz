...
try {
    ...
    // 2. Verificar total de Fichas SocioeconÃ´micas
    echo "<h3>Total de Fichas SocioeconÃ´micas:</h3>";
    ...
    foreach ($fichas as $ficha) {
        $status = !empty($ficha['idficha']) ? "âœ“ COM FICHA" : "âœ— SEM FICHA";
        echo "[{$status}] ID: {$ficha['idatendido']} | Nome: {$ficha['nome']} | Renda: {$ficha['renda_familiar']} | Qtd Pessoas: {$ficha['qtd_pessoas']} | Nr CÃ´modos: {$ficha['nr_comodos']}\n";
    }
    
    echo "</pre>";
    echo "<hr>";
    
    // Mostrar tambÃ©m o JSON completo
    ...
    echo "<hr>";
    echo "<h3>Atendidos SEM Ficha SocioeconÃ´mica:</h3>";
    ...
    if (!empty($semFicha)) {
        ...
        echo "<p>Todos os atendidos tÃªm ficha socioeconÃ´mica!</p>";
    }
