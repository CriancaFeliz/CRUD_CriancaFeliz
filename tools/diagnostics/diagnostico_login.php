 ...
 * Acesse: http://localhost/CriancaFeliz/tools/diagnostics/diagnostico_login.php
 */
...
try {
    ...
    echo "<h2>ðŸ” DiagnÃ³stico de Login</h2>";
    ...
    if ($user) {
        echo "<h3>âœ… UsuÃ¡rio encontrado no banco</h3>";
        ...
        echo "<tr><td><strong>NÃ­vel:</strong></td><td>{$user['nivel']}</td></tr>";
        ...
        if ($senhaCorreta) {
            ...
            echo "<p>Execute o script de correÃ§Ã£o:</p>";
            echo "<p><a href='../maintenance/fix_users_mysql.php' style='background:#f0a36b; color:white; padding:10px 20px; text-decoration:none; border-radius:6px;'>ðŸ”§ Corrigir Senhas</a></p>";
        }
        ...
        echo "<p><strong>Teste do novo hash:</strong> " . ($testeNovoHash ? 'âœ… OK' : 'âŒ ERRO') . "</p>";
        
    } else {
        echo "<h3>âŒ UsuÃ¡rio NÃƒO encontrado no banco</h3>";
        echo "<p>O email <strong>$email</strong> nÃ£o existe na tabela Usuario.</p>";
        echo "<p><a href='../maintenance/fix_users_mysql.php' style='background:#f0a36b; color:white; padding:10px 20px; text-decoration:none; border-radius:6px;'>âž• Criar UsuÃ¡rios</a></p>";
    }
    
    echo "<hr>";
    echo "<h3>ðŸ“‹ Todos os UsuÃ¡rios no Banco</h3>";
    ...
    if (count($todos) > 0) {
        ...
        echo "<p>Nenhum usuÃ¡rio encontrado no banco.</p>";
    }
    
} catch (PDOException $e) {
    echo "<h2>âŒ Erro ao conectar ao banco</h2>";
